  ...
  context 'fetching new messages' do
    ...
    before(:each) do
      ...
      3.times do |n|
        ...
        message = FactoryBot.create(:spoke_message_errored, created_at: @time, id: n+3, assignment: spoke_assignment, user_number: @spoke_user.cell, contact_number: campaign_contact.cell, user_number: @spoke_user.cell)
    ...
    it 'should record contactee and contactor details' do
      IdentitySpoke.fetch_new_messages
      contactee_phone_number = PhoneNumber.where(phone: IdentitySpoke::CampaignContact.first.cell.sub(/^[+]*/,''))
      contactee = Member.find(contactee_phone_number.member_id)
      contactor_phone_number = PhoneNumber.where(phone: @spoke_user.cell.sub(/^[+]*/,''))
      contactor = Member.find(contactor_phone_number.member_id)
      expect(Contact.find_by_external_id('1')).to have_attributes(contactee_id: contactee.id, contactor_id: contactor.id)
